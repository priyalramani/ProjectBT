	...
	const onSubmit = async () => {
		let item_details = order.item_details
			.map(item => ({
				...item,
				b: 0,
				p: (+item.b || 0) * (+item.conversion || 0) + (+item.p || 0) - (+item.qty || 0)
			}))
			.filter(a => a.p)
		...
		if (order.to_warehouse)
			setOrder(prev => ({
				...
				item_details: itemsData
					...
					.map(a => {
						let qty = a?.stock?.find(b => b.warehouse_uuid === order.to_warehouse)?.qty || 0
									...
									<Select
										...
										options={warehouse
											?.filter(
												a =>
													!counterFilter ||
													a.warehouse_title?.toLocaleLowerCase()?.includes(counterFilter.toLocaleLowerCase())
											)
											.map(a => ({
									...
									<tbody className="lh-copy">
										{order?.item_details
											?.filter(
												a =>
													!filterItems ||
													a.item_title.toLocaleLowerCase().includes(filterItems.toLocaleLowerCase())
											)
											.map((item, i) => (
												<tr key={item.uuid}>
														...
														<input
															...
															onChange={e =>
																setOrder(prev => ({
																	...prev,
																	item_details: prev.item_details.map(a =>
																		a.uuid === item.uuid ? { ...a, remarks: e.target.value || null } : a
																	)
																}))
															}
														/>
													...
													<td className="ph2 pv1 tc bb b--black-20 bg-white" style={{ textAlign: "center" }}>
														{CovertedQty(
															(+item.b || 0) * (+item.conversion || 0) + (+item.p || 0) - (+item.qty || 0),
															item.conversion
														)}
													</td>
												</tr>
					...
					>
						<thead>
							<tr>
								...
								>
									In: {warehouse.find(a => a.warehouse_uuid === order.to_warehouse)?.warehouse_title}
								</th>
							</tr>
							<tr>
...
export function SuggestionsPopup({ onSave, warehouse, itemsData, order, setOrder }) {
...
function Table({ itemsDetails, warehouse_uuid, selectedOrders, setSelectedOrders }) {
