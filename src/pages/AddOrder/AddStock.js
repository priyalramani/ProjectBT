																	...
																	item_details: prev.item_details.map(a => {
																		if (a.uuid === item.uuid) {
																			...
																			return {
																				...
																				b_price: Math.floor(
																					item.item_price * item.conversion ||
																						0
																				),
																			}
																		} else return a
					...
					<table
						...
						<thead>
							...
							<tr>
								<th
									...
									From:{" "}
									{warehouse.find(a => a.warehouse_uuid === order.from_warehouse)?.warehouse_title ||
										"None"}
								</th>
								<th
									colSpan={3}
						...
						<tbody className="tbody">
							{category
								...
								.filter(
									a => order?.item_details?.filter(b => a.category_uuid === b.category_uuid).length
								)
								.map(a => (
									<>
		...
		<table className="user-table" style={{ maxWidth: "100vw", height: "fit-content", overflowX: "scroll" }}>
			<thead>
				<tr>
					...
					{order.from_warehouse ? (
						<th colSpan={2}>
							{warehouseData.find(a => a.warehouse_uuid === order?.from_warehouse)?.warehouse_title}
						</th>
					) : (
						""
					)}
					<th colSpan={2}>
						{warehouseData.find(a => a.warehouse_uuid === order?.to_warehouse)?.warehouse_title}
					</th>
				</tr>
			</thead>
			<tbody className="tbody">
									...
									<span
										...
										<input
											...
											checked={
												...
												itemsDetails?.filter(b => a.category_uuid === b.category_uuid).length
											}
