...
const ItemPopup = ({ onSave, itemPopupId, items, objData, setObgData }) => {
	...
	const [filterItemData, setFilterItemData] = useState([])
	useEffect(() => {
		setValue(objData?.qty_details?.find(a => a.uuid === itemPopupId)?.add_items || [])
		setFilterItemData(
			items.sort((a, b) => {
				let aLength = objData?.qty_details
					?.find(a => a.uuid === itemPopupId)
					?.add_items?.filter(c => c.item_uuid === a.item_uuid)?.length
				let bLength = objData?.qty_details
					?.find(a => a.uuid === itemPopupId)
					?.add_items?.filter(c => c.item_uuid === b.item_uuid)?.length
				if (aLength && bLength) {
					return a.item_title?.localeCompare(b.item_title)
				} else if (aLength) {
					return -1
	...
	const submitHandler = () => {
		setObgData(prev => ({
			...prev,
			qty_details: prev.qty_details.map(a => (a.uuid === itemPopupId ? { ...a, add_items: value } : a)),
		}))
		onSave()
	}
