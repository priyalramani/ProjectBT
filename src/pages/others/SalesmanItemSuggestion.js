...
function UserPayouts({ onSave, popupInfo, getUsers }) {
  ...
  useEffect(() => {
    setFilterItemData(
      items.sort((a, b) => {
        let aLength = salesmanSuggestion.filter(
          (c) => c === a.item_uuid
        )?.length;
        let bLength = salesmanSuggestion.filter(
          (c) => c === b.item_uuid
        )?.length;
        if (aLength && bLength) {
    ...
    if (response.data.success)
      setItems(
        response.data.result.map((b) => ({
          ...b,
          company_title: companyData.find(
            (a) => a?.company_uuid === b?.company_uuid
          )?.company_title,
          category_title: ItemCategoryData.find(
