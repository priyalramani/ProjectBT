								...
								<Select
									...
									value={
										filterCompany
											? {
													value: filterCompany,
													label: companies?.find(j => j.company_uuid === filterCompany)
														?.company_title,
											  }
											: { value: "", label: "All" }
									}
									...
									value={
										filterCategory
											? {
													...
													label: itemCategories?.find(j => j.category_uuid === filterCategory)
														?.category_title,
											  }
											: { value: "", label: "All" }
									}
									...
									<Selected
										...
										{warehouseData.map(variant => (
											<MenuItem key={variant.id} value={variant} style={{ zIndex: 99999999999 }}>
												<Checkbox
													checked={data.findIndex(item => item.id === variant.id) >= 0}
												/>
												<ListItemText placeholder="variant.name" primary={variant.name} />
											</MenuItem>
										))}
