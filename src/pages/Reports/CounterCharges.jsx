...
const CounterCharges = () => {
  ...
  const fetchCompanies = async () => {
    try {
      ...
      const response = await axios.get(
        `/counterCharges/${completedFilter ? "completed" : "running"}`
      );
  ...
  const updateCharge = async (charge) => {
    try {
      ...
      if (response?.data?.success)
        setCharges((state) =>
          state?.map((i) =>
            i.charge_uuid !== charge?.charge_uuid
              ? i
              : { ...i, ...response.data.result }
          )
        );
      setPageLoading(false);
    } catch (error) {
  ...
  const deleteCharge = async (charge) => {
    ...
    try {
      ...
      setCharges((state) =>
        state?.filter((i) => i.charge_uuid !== charge?.charge_uuid)
      );
      setPageLoading(false);
    } catch (error) {
      setPageLoading(false);
  ...
  return (
    <>
      ...
      <div className="layout-content" id="companies-wrapper">
        ...
        <div id="companies-header">
          ...
          <div id="actions-wrapper">
            <div
              id="completed-filter"
              onClick={() => setCompletedFilter((i) => !i)}
            >
              {completedFilter ? (
                <ImCheckboxChecked fill="#000000" />
              ) : (
                <ImCheckboxUnchecked fill="#000000" />
              )}
            ...
            <button
              ...
              onClick={() =>
                setFormState({ active: true, onSubmit: postCharge, counters })
              }
            >
            ...
            <tbody>
              {filterCharges(charges)?.map((charge, index) => (
                <tr key={charge?.charge_uuid}>
                  ...
                  <td>
                    {charge?.completed_at
                      ? new Date(charge?.completed_at)?.toDateString()
                      : "N/A"}
                  </td>
                  <td>
      ...
      {formState?.active && (
        <CompanyForm
          {...formState}
          close={() => setFormState((i) => ({ ...i, active: false }))}
        />
      )}
    </>
  );
...
const CompanyForm = ({ close, onSubmit, data = {}, counters }) => {
  ...
  const onChange = (e) =>
    setCharge((i) => ({ ...i, [e.target.name]: e.target.value }));
  const submit = (e) => {
    e.preventDefault();
    onSubmit(charge);
    close();
  ...
  const FormFields = [
    ...
    {
      label: "Narration (Displayed)",
      key: "narration",
      attributes: { maxlength: 15, placeholder: "Displayed in invoice" },
    },
    {
      label: "Remarks (Not displayed)",
      key: "remarks",
